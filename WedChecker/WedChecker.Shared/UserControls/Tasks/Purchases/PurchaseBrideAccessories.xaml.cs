    ...
    public sealed partial class PurchaseBrideAccessories : BaseTaskControl
    {
        ...
        public List<string> StoredAccessories
        {
            get
            {
                ...
                {
                    _storedAccessories = AppData.GetStorage("BrideAccessories") as List<string>;
                }

                if (_storedAccessories == null || !_storedAccessories.Any())
                {
                    throw new WedCheckerInvalidDataException("You must first add bride accessories in order to mark them purchased after that!");
                }
